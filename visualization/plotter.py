...
def plot_domain_geometry(ax, X, Y, Z, alpha=1.0):
    ax.clear()
...
def plot_cutaway3d(ax, X, Y, Z, T, z_idx, Lx, Ly, Lz, alpha=1.0):
    ax.clear()
    ...
    surf_top = ax.plot_surface(X[::sy, ::sx, z_idx], Y[::sy, ::sx, z_idx], Z[::sy, ::sx, z_idx], 
                               facecolors=cmap(norm(T[::sy, ::sx, z_idx])), shade=False, linewidth=0, antialiased=True, alpha=alpha)
                     
    # 3. Bottom Face (Z = 0)
    ax.plot_surface(X[::sy, ::sx, 0], Y[::sy, ::sx, 0], Z[::sy, ::sx, 0], 
                    facecolors=cmap(norm(T[::sy, ::sx, 0])), shade=False, linewidth=0, antialiased=True, alpha=alpha)
                
    # 4. Front Face (Y = 0)
    ax.plot_surface(X[0, ::sx, :z_idx+1:sz], Y[0, ::sx, :z_idx+1:sz], Z[0, ::sx, :z_idx+1:sz], 
                    facecolors=cmap(norm(T[0, ::sx, :z_idx+1:sz])), shade=False, linewidth=0, antialiased=True, alpha=alpha)
                
    # 5. Back Face (Y = Ly)
    ax.plot_surface(X[-1, ::sx, :z_idx+1:sz], Y[-1, ::sx, :z_idx+1:sz], Z[-1, ::sx, :z_idx+1:sz], 
                    facecolors=cmap(norm(T[-1, ::sx, :z_idx+1:sz])), shade=False, linewidth=0, antialiased=True, alpha=alpha)
                
    # 6. Left Face (X = 0)
    ax.plot_surface(X[::sy, 0, :z_idx+1:sz], Y[::sy, 0, :z_idx+1:sz], Z[::sy, 0, :z_idx+1:sz], 
                    facecolors=cmap(norm(T[::sy, 0, :z_idx+1:sz])), shade=False, linewidth=0, antialiased=True, alpha=alpha)
                
    # 7. Right Face (X = Lx)
    ax.plot_surface(X[::sy, -1, :z_idx+1:sz], Y[::sy, -1, :z_idx+1:sz], Z[::sy, -1, :z_idx+1:sz], 
                    facecolors=cmap(norm(T[::sy, -1, :z_idx+1:sz])), shade=False, linewidth=0, antialiased=True, alpha=alpha)
